...
def get_likelihoods(learned_params):
...
full_learned_params = [pickle.load(open("{}/{}/{}/full_data_post_shard_{}.p".format(folder, experiment, approximate, i), "rb")) for i in range(num_shards)]
full_removed_likelihood_np, full_remain_likelihood_np = get_likelihoods(full_learned_params)

print("Likelihood of re-trained posterior"); sys.stdout.flush()
retrain_learned_params = [pickle.load(open("{}/{}/{}/remain_data_retrain_post_shard_{}.p".format(folder, experiment, approximate, i), "rb")) for i in range(num_shards)]
retrain_removed_likelihood_np, retrain_remain_likelihood_np = get_likelihoods(retrain_learned_params)
...
for i,percentage in enumerate(elbo_mode_percentages):
    ...
    elbo_unlearned_params = [pickle.load(open("{}/{}/{}/data_remain_data_by_unlearn_elbo_shard_{}_mode_{}.p".format(folder,experiment, approximate, j, percentage), "rb")) for j in range(num_shards)]
    removed_likelihood_i, remain_likelihood_i = get_likelihoods(elbo_unlearned_params)
...
for i,percentage in enumerate(eubo_mode_percentages):
    ...
    eubo_unlearned_params = [pickle.load(open("{}/{}/{}/data_remain_data_by_unlearn_eubo_shard_{}_mode_{}.p".format(folder,experiment, approximate, j, percentage), "rb")) for j in range(num_shards)]
    removed_likelihood_i, remain_likelihood_i = get_likelihoods(eubo_unlearned_params)
