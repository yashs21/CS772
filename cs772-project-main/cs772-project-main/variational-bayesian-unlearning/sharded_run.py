...
if __name__ == "__main__":
    ...
    sharded_data = np.array_split(data, num_shards)
    ...
    # Train a model on each shard
    learned_param_full_shards = []
    learned_param_remain_shards = []
    for i in range(num_shards):
        ...
        filename = '{}/full_data_post_shard_{}.p'.format(path, i)
        learned_param_full = utils.load_file_if_exist_else_create(filename, 
                lambda : unlearn.learn(sharded_data[i], ntrain=ntrain, batchsize=batchsize), 
                force_create_new = False)
        learned_param_full_shards.append(learned_param_full)

        filename = '{}/remain_data_retrain_post_shard_{}.p'.format(path, i)
        learned_param_remain = utils.load_file_if_exist_else_create(filename, 
                lambda : unlearn.learn(remain_data, 
                                ...
                                batchsize=batchsize), 
                force_create_new = False)
        learned_param_remain_shards.append(learned_param_remain)

    # Find the mode of the distribution using the first shard
    approximate_dist_instance = approximate_dist(nparam, **approximate_config)
    mode_input, mode_lprob = approximate_dist_instance.find_mode(param=learned_param_full_shards[0], n_init=10, ntrain=10000)
    ...
    for mode_percentage in eubo_mode_percentages:
        all_eubo_unlearned_param[mode_percentage] = []
        for i in range(num_shards):
            ...
            filename = '{}/data_remain_data_by_unlearn_eubo_shard_{}_mode_{}.p'.format(path, i, mode_percentage)
            unlearned_param = utils.load_file_if_exist_else_create(filename, 
                    lambda : unlearn.unlearn_EUBO(removed_data, 
                                    learned_param_full_shards[i], 
                                    log_threshold=log_threshold, 
                                    ntrain=ntrain,
                                    batchsize=batchsize), 
                    force_create_new = False)
            all_eubo_unlearned_param[mode_percentage].append(unlearned_param)

    for mode_percentage in elbo_mode_percentages:
        all_elbo_unlearned_param[mode_percentage] = []
        for i in range(num_shards):
            ...
            filename = '{}/data_remain_data_by_unlearn_elbo_shard_{}_mode_{}.p'.format(path, i, mode_percentage)
            unlearned_param = utils.load_file_if_exist_else_create(filename, 
                    lambda : unlearn.unlearn_ELBO(removed_data, 
                                    ...
                                    batchsize=batchsize), 
                    force_create_new = False)
            all_elbo_unlearned_param[mode_percentage].append(unlearned_param)